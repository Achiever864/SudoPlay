...
export default function LeaderboardTab() {
    ...
    const getPodiumOrderStyles = (rank: number) => {
        if (rank === 1)
            return { order: "order-2", height: "h-36 sm:h-40", border: "border-amber-400 bg-amber-500/10", text: "text-amber-400", label: "1st" };
        if (rank === 2)
            return { order: "order-1", height: "h-28 sm:h-32", border: "border-slate-300 bg-slate-300/10", text: "text-slate-300", label: "2nd" };
    ...
    return (
        <div className="w-full max-w-lg flex flex-col gap-6">
            {/* Scope toggle */}
            <div className="w-full bg-slate-800/40 backdrop-blur-md p-1.5 rounded-xl border border-slate-700/50 shadow-lg flex">
                <button
                ...
                {podiumPlayers.map((player) => {
                    ...
                    return (
                        <div key={player.username} className={`flex-1 flex flex-col items-center ${style.order}`}>
                            ...
                            <div
                                className={`w-full ${style.height} rounded-t-2xl border-t border-x ${style.border} flex flex-col items-center justify-center relative shadow-2xl`}
                            >
                ...
                {listPlayers.map((player) => (
                    ...
                    >
                        ...
                        <div className="text-right">
                            <div className="font-mono text-sm font-bold text-cyan-400">+{player.xp} XP</div>
                            <div className="text-[10px] font-mono text-slate-500 mt-0.5">Avg: {player.time}</div>
                        </div>
                    </div>
                ))}
            </div>
